...
const AbsensiPulang = () => {
  ...
  // Sesuai koordinat testing kamu
  const OFFICE_COORDS = useMemo(() => ({ lat: -6.244867901200337, lng: 106.87230989592847 }), []); //-6.244867901200337, 106.87230989592847
  
  // LOGIKA: Batas Pulang (16:30 s/d 23:59)
  const isOutsideReturnWindow = useMemo(() => {
    const now = new Date();
    const hours = now.getHours();
    const minutes = now.getMinutes();
    // Return true jika jam < 16:30
    const tooEarly = hours < 16 || (hours === 16 && minutes < 30);
    const isNextDay = hours === 0; 

    return tooEarly || isNextDay;
  }, []);

  const videoRef = useRef(null);
  const canvasRef = useRef(null);

  // FUNGSI KIRIM DATA PULANG KE LARAVEL
  const submitAbsenPulang = async () => {
    if (isOutsideReturnWindow) return alert("Belum waktunya pulang! (Minimal 16:30)");
    ...
    try {
        ...
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/absen-pulang`, payload, {
  ...
  useEffect(() => {
    if (step === 1) {
      const watchId = navigator.geolocation.watchPosition((pos) => {
        ...
        setIsWithinRange(d <= 100); // Toleransi 100m untuk testing
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <header className="bg-white/80 backdrop-blur-md sticky top-0 z-[1000] p-4 md:px-10 flex items-center justify-between border-b border-slate-100">
          ...
          <div className="hidden md:flex bg-orange-50 px-4 py-2 rounded-xl items-center gap-3">
              ...
              <span className="text-xs font-bold text-orange-800 uppercase">GPS: {accuracy}m</span>
          </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-10">
        ...
        {step === 1 && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500">
            <div className="lg:col-span-2 h-[400px] md:h-[600px] rounded-[2.5rem] overflow-hidden shadow-inner border border-slate-200 z-0">
              {location && (
                <MapContainer center={[location.lat, location.lng]} zoom={17} zoomControl={false} className="h-full w-full grayscale-[0.3]">
            ...
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-xl border border-orange-100 flex flex-col justify-center text-center space-y-8">
                ...
                <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight">
                        {isWithinRange ? 'Siap Pulang' : 'Diluar Jangkauan'}
                    </h2>
                    <p className="text-sm font-medium text-slate-400 mt-2 px-4">
                        {isWithinRange ? 'Lokasi aman. Silakan ambil foto selfie untuk pulang.' : `Jarak: ${distance}m dari titik kantor.`}
                    </p>
                </div>
                <button 
                  disabled={!isWithinRange}
                  onClick={startCamera}
                  className={`w-full py-5 rounded-3xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${isWithinRange ? 'bg-orange-600 text-white shadow-xl shadow-orange-200 active:scale-95' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
                >
        ...
        {step === 2 && (
          <div className="fixed inset-0 bg-slate-900 z-[2000] flex flex-col items-center justify-center p-4">
            ...
            <div className="mt-10 flex gap-6">
                ...
                <button onClick={capturePhoto} className="p-10 bg-orange-600 rounded-full text-white shadow-2xl hover:scale-110 active:scale-90 border-8 border-white/20">
        ...
        {step === 3 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-bottom-10 duration-500">
            <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-orange-600 to-amber-600 rounded-[3rem] blur opacity-20 transition duration-1000"></div>
                <div className="relative bg-white p-4 rounded-[3rem] shadow-2xl">
                    <img src={photo} className="w-full h-[400px] md:h-[500px] object-cover rounded-[2.5rem]" alt="Selfie" />
                </div>
            </div>
            ...
            <div className="flex flex-col justify-center space-y-8">
                ...
                {isOutsideReturnWindow ? (
                    <div className="bg-rose-50 border border-rose-100 p-8 rounded-[2rem] flex items-center gap-5 text-rose-600">
                        <AlertTriangle className="w-8 h-8" />
                        <div>
                            <p className="font-black uppercase text-sm">Diluar Jam Operasional!</p>
                            <p className="text-xs font-bold">Pulang hanya bisa dilakukan antara 16:30 s/d 23:59.</p>
                        </div>
                    </div>
                ) : (
                    <div className="bg-orange-50 border border-orange-100 p-8 rounded-[2rem] flex items-center gap-5">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <button 
                        onClick={submitAbsenPulang}
                        disabled={loading || isOutsideReturnWindow}
                        className="flex-1 py-5 bg-orange-600 text-white rounded-3xl font-black uppercase tracking-[0.2em] shadow-xl shadow-orange-100 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
                    >
                        {loading ? <Loader2 className="animate-spin" /> : 'Konfirmasi Pulang'}
                    </button>
