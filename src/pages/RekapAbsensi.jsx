...
import { 
  ...
  Eye, X, ImageOff // Tambah icon Eye & X
} from 'lucide-react';
...
const RekapAbsensi = () => {
  ...
  // 1. STATE UNTUK POPUP PREVIEW FOTO
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-10">
          ...
          <div className="overflow-x-auto text-nowrap">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50 font-medium">
                {loading ? (
                  <tr><td colSpan="6" className="py-24 text-center"><Loader2 className="animate-spin mx-auto text-blue-600 mb-2" /><p className="text-[10px] font-black text-slate-400 uppercase">Menyinkronkan...</p></td></tr>
                ) : data.length > 0 ? data.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-5">
                      {/* 2. PENYESUAIAN BE: Gunakan nama_karyawan */}
                    ...
                    <td className="px-8 py-5 text-center text-sm font-black text-slate-700">
                        {row.jam_masuk?.slice(0,5) || '--:--'} <span className="text-slate-300 mx-1">/</span> {row.jam_pulang?.slice(0,5) || '--:--'}
                    </td>
                    
                    {/* 3. FITUR PREVIEW FOTO */}
                    <td className="px-8 py-5 text-center">
                        <div className="flex justify-center gap-2">
                            {/* Tombol Foto Masuk */}
                            {row.foto_masuk_url ? (
                                <button onClick={() => setSelectedImage(row.foto_masuk_url)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all" title="Foto Masuk">
                            ...
                            {/* Tombol Foto Pulang */}
                            {row.foto_pulang_url ? (
                                <button onClick={() => setSelectedImage(row.foto_pulang_url)} className="p-2 bg-orange-50 text-orange-600 rounded-lg hover:bg-orange-600 hover:text-white transition-all" title="Foto Pulang">
                                    <Eye className="w-4 h-4" />
                                </button>
                            ) : null}
                        </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-2000 flex items-center justify-center p-4 md:p-8">
            ...
            {/* Image Container */}
            <div className="relative bg-white p-2 rounded-[2.5rem] shadow-2xl max-w-lg w-full animate-in zoom-in duration-300">
                <button 
                    onClick={() => setSelectedImage(null)}
                    className="absolute -top-4 -right-4 bg-rose-600 text-white p-3 rounded-full shadow-xl hover:bg-rose-700 transition-all z-50"
                >
                    <X className="w-6 h-6" />
                </button>
                <img 
                ...
                <div className="p-6 text-center">
                    <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Bukti Verifikasi Biometrik</p>
                </div>
            </div>
        </div>
      )}
