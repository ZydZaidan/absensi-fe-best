...
const AdminDashboard = () => {
  ...
  // --- NEW STATE: Hitung pengajuan pending untuk badge ---
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        // Panggil 3 API sekaligus: Status Absen, Riwayat, dan Daftar Pengajuan Pulang Cepat
        const [resStatus, resHistory, resPendingPulang] = await Promise.all([
        ...
        // Simpan jumlah pengajuan yang ada di antrean
        setPendingPulangCount(resPendingPulang.data.data?.length || 0);
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-8">
        ...
        <section className="bg-linear-to-br from-slate-800 to-slate-900 rounded-[2.5rem] p-10 text-white shadow-xl relative overflow-hidden">
          <div className="flex flex-row justify-between items-center gap-6">
            <div>
              <div className="bg-blue-600 w-fit px-3 py-1 rounded-full mb-3 shadow-lg shadow-blue-900/20">
          ...
          <div className="mt-10 bg-white/5 backdrop-blur-md border border-white/10 rounded-4xl p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-black tracking-tight">{todayData?.jam_masuk?.slice(0,5) || '-- : --'}</p>      
              </div>
              <div className="space-y-1">
                <p className="text-[10px] uppercase font-bold opacity-60 tracking-widest">Jam Pulang</p>
                <p className="text-2xl font-black tracking-tight opacity-30">{todayData?.jam_pulang?.slice(0,5) || '-- : --'}</p>
              </div>
            </div>
          </div>
        </section>

        {/* 3. STATISTIK DINAMIS (Pribadi Admin) */}
        <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
          <div className="flex justify-around items-center">
            ...
            <div className="w-px h-10 bg-slate-100 shrink-0"></div>
            <div className="text-center flex-1">
              <p className="text-[10px] font-black uppercase tracking-widest mb-1 text-rose-400">Terlambat</p>
        ...
        {/* 4. ACTION BUTTONS GRID (5 Tombol) */}
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          
          {/* TOMBOL ABSEN ADMIN (Dinamis) */}
          {!todayData ? (
            <button onClick={() => navigate('/absensi')} className="flex flex-col items-center justify-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-50 hover:border-blue-500 transition-all group active:scale-95">
            ...
            <button disabled className="flex flex-col items-center justify-center bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200 opacity-60">
            ...
            <button disabled className="flex flex-col items-center justify-center bg-amber-50 p-8 rounded-[2.5rem] border border-amber-100 opacity-80 cursor-wait">
                <div className="bg-amber-100 p-4 rounded-2xl mb-4 animate-pulse"><Clock className="text-amber-600" /></div>
                <span className="text-[9px] font-black text-amber-700 uppercase text-center leading-tight">Menunggu <br/> ACC Admin</span>
            </button>
          ) : (
            <button onClick={() => navigate('/absensi-pulang')} className="flex flex-col items-center justify-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-orange-100 hover:border-orange-500 transition-all group active:scale-95">
          ...
          {/* IZIN / SAKIT PRIBADI */}
          <button onClick={() => navigate('/izin')} className="flex flex-col items-center justify-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-50 hover:border-emerald-500 transition-all group active:scale-95">
          ...
          <button onClick={() => navigate('/admin/verifikasi')} className="flex flex-col items-center justify-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-50 hover:border-indigo-500 transition-all group active:scale-95">
            ...
            <span className="text-[10px] font-black text-indigo-900 uppercase tracking-widest text-center">Verifikasi Karyawan</span>
          </button>

          {/* PERSETUJUAN PULANG CEPAT (With Badge) */}
          <button onClick={() => navigate('/admin/persetujuan-pulang-cepat')} className="relative flex flex-col items-center justify-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-orange-50 hover:border-orange-500 transition-all group active:scale-95">
            {pendingPulangCount > 0 && (
                <div className="absolute top-6 right-8 w-6 h-6 bg-rose-600 text-white rounded-full flex items-center justify-center text-[10px] font-black animate-bounce shadow-lg border-4 border-white">
                    {pendingPulangCount}
                </div>
            )}
            ...
            <span className="text-[10px] font-black text-orange-900 uppercase tracking-widest text-center leading-tight">Persetujuan <br/> Pulang</span>
          </button>

          {/* REKAP ABSENSI GLOBAL */}
          <button onClick={() => navigate('/admin/rekap-absen')} className="flex flex-col items-center justify-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-50 hover:border-purple-500 transition-all group active:scale-95">
            ...
            <span className="text-[10px] font-black text-purple-900 uppercase tracking-widest text-center leading-tight">Rekapitulasi <br/> Absensi</span>
          </button>

        </section>

        {/* 5. HISTORY LIST (Hanya 3 Teratas) */}
        <section className="space-y-6 pt-4">
          ...
          <div className="space-y-4">
            {history.length > 0 ? history.slice(0, 3).map((item, i) => (
              ...
              <p className="text-center text-slate-300 font-bold uppercase text-[10px] py-10">Belum ada aktivitas tercatat.</p>
            )}
          </div>
        </section>
